...
namespace pozdeev {
  ...
  {
    HashNode< Key, Value > ** newTable = new HashNode< Key, Value >*[slots];
    ...
    for (size_t i = 0; i < capacity_; ++i) {
      ...
      while (current != nullptr) {
        HashNode< Key, Value > * nextNode = current->next_;
        size_t index = hasher_(current->key_) % slots;
        current->next_ = newTable[index];
  ...
  {
    return Iterator(table_, capacity_, 0, nullptr);
  }

  template< class Key, class Value, class Hash, class Equal >
  typename HashTable< Key, Value, Hash, Equal >::Iterator HashTable< Key, Value, Hash, Equal >::end()
  {
    return Iterator(table_, capacity_, capacity_, nullptr);
  }
